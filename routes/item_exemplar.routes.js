...
const item_exemplar = require('../model/item_exemplar');
...
router.post('/', async (req, res) => {
    try {
        const query = `INSERT INTO itens (emprestimoId, livroId, quantidade, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?)`;
        const replacements = [req.body.emprestimoId, req.body.livroId, req.body.quantidade, new Date(), new Date()];
...
//método DELETE para deletar através do id um item dos exemplares
router.delete('/:id', async (req, res) => {
    try {
        const { id } = req.params; //pega o id enviado pela requisição para ser excluído
    
        await sequelize.query("DELETE FROM itens WHERE id = ?",
            {
                replacements: [id],
                type: QueryTypes.DELETE
            });
        res.status(200).json({ message: 'Item deletado com sucesso.' }); //statusCode indica ok no delete
    } catch (error) {
        res.status(400).json({ msg: error.message }); //retorna status de erro e mensagens
    }
});
